...
export class SapiTtsService {
  private currentPs: ChildProcess | null = null
  ...
  cancel(): void {
    ...
    this.queue = []
    // Kill the in-flight PowerShell process immediately for fast barge-in
    if (this.currentPs) {
      try {
        this.currentPs.kill('SIGTERM')
      } catch {
        /* ignore */
      }
      this.currentPs = null
...
$synth.Speak('${safeText}')
      ...
      ps.on('close', (code) => {
        if (this.currentPs === ps) this.currentPs = null
